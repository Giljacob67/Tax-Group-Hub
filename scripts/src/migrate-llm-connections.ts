...
async function migrate() {
  ...
  const configs = await db
    .select()
    .from(appConfigTable)
    .where(eq(appConfigTable.key, "ACTIVE_LLM_PROVIDER"));
  const activeProvider =
    configs[0]?.value || process.env.ACTIVE_LLM_PROVIDER || "auto";
  const activeModel =
    (
      await db
        .select()
  ...
  if (!key && providerToUse !== "ollama") {
    ...
    console.log(
      "   Migration skipped — user will need to set up connections manually in the UI.",
    );
    return;
  }
