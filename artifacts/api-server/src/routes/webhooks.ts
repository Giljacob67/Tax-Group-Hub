  ...
  try {
    ...
    if (isLegacyToken) {
      ...
      [config] = await db
        ...
        .where(
          and(
            eq(channelConfigsTable.platform, "telegram"),
            eq(channelConfigsTable.externalId, webhookId),
          ),
        )
        .limit(1);
    } else {
      ...
      if (!isNaN(configId)) {
        [config] = await db
          ...
          .where(
            and(
              eq(channelConfigsTable.platform, "telegram"),
              eq(channelConfigsTable.id, configId),
            ),
          )
          .limit(1);
      }
    ...
    let [conv] = await db
      ...
      .where(
        and(
          eq(conversationsTable.platform, "telegram"),
    ...
    const [config] = await db
      ...
      .where(
        and(
          ...
          eq(channelConfigsTable.id, numericId),
        ),
      )
      .limit(1);
...
router.post("/webhooks/whatsapp/:channelId", async (req, res) => {
  ...
  try {
    ...
    let [conv] = await db
      ...
      .where(
        and(
          eq(conversationsTable.platform, "whatsapp"),
          eq(conversationsTable.externalId, fromNumber),
        ),
      )
      .limit(1);
    ...
    if (msg.type === "text" && msg.text) {
      ...
      const mediaUrl = await resolveWhatsAppMediaUrl(
        msg.document.id,
        accessToken,
      );
      if (mediaUrl) {
    ...
    await db
      ...
      .values({ conversationId: conv.id, role: "user", content: userContent });
...
router.post("/webhooks/crm/inbound", async (req, res) => {
  ...
  const expectedSecret =
    process.env.CRM_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET;
  const providedSecret =
    req.headers["x-crm-webhook-secret"] || req.headers["x-webhook-secret"];
  if (
    !expectedSecret ||
  ...
  if (
    ...
    tenantId.trim() === "" ||
    ["system", "default", "demo-user"].includes(tenantId)
  ) {
  ...
  try {
    ...
    let [contact] = await db
      ...
      .where(
        and(
          eq(crmContactsTable.cnpj, cnpjValid),
          eq(crmContactsTable.userId, tenantId),
        ),
      )
      .limit(1);
