...
const FINALIZADO_CONTATO = new Set([
  "cliente",
  "perdido",
  "stand_by",
  "encerrado",
]);
const FINALIZADO_DEAL = new Set([
  "fechado_ganho",
  "perdido",
  "stand_by",
  "encerrado",
...
export async function getExecutiveDashboard(userId: string, period: string) {
  ...
  // Per-period metrics
  const newLeadsInPeriod = contacts.filter(
    (c) => new Date(c.createdAt) >= start,
  ).length;
  const leadsQualificados = contacts.filter(
    (c) => c.status === "qualificado",
  ).length;
  const meetingsToday = tasks.filter(
    (t) =>
      t.type === "meeting" &&
      t.dueDate &&
  ...
  const meetingsThisWeek = tasks.filter(
    (t) =>
      t.type === "meeting" &&
      t.dueDate &&
  ...
  const qualificationRate =
    contacts.length > 0
      ? Math.round((leadsQualificados / contacts.length) * 100)
      : 0;
  const winRate =
  ...
  // Matriz metrics
  const matrizEnviado = deals.filter(
    (d) => d.statusMatriz === "enviado" || d.statusMatriz === "aguardando",
  ).length;
  const matrizAguardando = deals.filter(
    (d) => d.statusMatriz === "aguardando",
  ).length;
  ...
  const avgDaysAwaitingMatriz = avgDays(
    ...
    (d) =>
      Math.floor(
        (Date.now() - new Date(d.dataEnvioMatriz!).getTime()) / DAY_MS,
      ),
  );
  ...
  const stuckOpportunities = activeDeals.filter(
    (d) => new Date(d.updatedAt) < fourteenDaysAgo,
  ).length;

  // Follow-ups vencidos
  const followupsVencidos = contacts.filter(
    (c) =>
      c.proximoFollowup &&
      new Date(c.proximoFollowup) < new Date() &&
...
export async function getCoordinatorDashboard(userId: string, period: string) {
  ...
  const byResponsible = new Map<
    string,
    { contacts: number; deals: number; value: number; hot: number }
  >();
  for (const c of contacts) {
    const key = c.responsavelUnidade || "(sem responsável)";
    const e = byResponsible.get(key) || {
      contacts: 0,
      deals: 0,
      value: 0,
      hot: 0,
  ...
  const criticalNegotiations = deals.filter(
    (d) => d.stage === "em_negociacao" && (d.probability || 0) >= 60,
  );

  // Proposals without return
  const now = new Date();
  const sevenDaysAgo = new Date(now.getTime() - 7 * DAY_MS);
  const proposalsNoReturn = deals.filter(
    (d) =>
      (d.statusProposta === "enviada" || d.statusProposta === "apresentada") &&
      new Date(d.updatedAt) < sevenDaysAgo,
  );

  // Inactive accounts
  const accountsInactive = contacts.filter(
    (c) =>
      !FINALIZADO_CONTATO.has(c.status) &&
      c.ultimaInteracao &&
  ...
  const upcomingFollowups = contacts
    ...
    .sort(
      (a, b) =>
        new Date(a.proximoFollowup!).getTime() -
        new Date(b.proximoFollowup!).getTime(),
    );

  // Bottlenecks (deals stuck in a stage)
  const stageBreakdown: Record<
    string,
    { count: number; avgDays: number; value: number }
  > = {};
  for (const d of deals) {
    if (FINALIZADO_DEAL.has(d.stage)) continue;
    const days = Math.floor(
      (now.getTime() - new Date(d.updatedAt).getTime()) / DAY_MS,
    );
    if (!stageBreakdown[d.stage])
      stageBreakdown[d.stage] = { count: 0, avgDays: 0, value: 0 };
  ...
  return {
    ...
    upcomingFollowups: upcomingFollowups.slice(0, 15).map((c) => ({
      id: c.id,
      name: c.razaoSocial,
      followup: c.proximoFollowup,
      status: c.status,
    })),
    awaitingMatriz: deals.filter(
      (d) => d.statusMatriz === "enviado" || d.statusMatriz === "aguardando",
    ).length,
    awaitingAction: deals.filter(
      (d) =>
        (d.statusMatriz === "enviado" || d.statusMatriz === "aguardando") &&
        (!d.briefingMatriz || d.briefingMatriz.trim().length < 10),
    ).length,
    stageBreakdown,
  };
}
...
export async function getOperationalDashboard(userId: string) {
  ...
  const followupsToday = contacts.filter((c) => {
    ...
    return (
      d >= todayStart && d <= todayEnd && !FINALIZADO_CONTATO.has(c.status)
    );
  });
  const followupsOverdue = contacts.filter(
    (c) =>
      c.proximoFollowup &&
      new Date(c.proximoFollowup) < now &&
      !FINALIZADO_CONTATO.has(c.status),
  ...
  // New leads (24h)
  const newLeads24h = contacts.filter(
    (c) => new Date(c.createdAt) >= new Date(now.getTime() - DAY_MS),
  ).length;

  // Accounts in each state
  const inApproach = contacts.filter((c) => c.status === "em_abordagem").length;
  const semResposta = contacts.filter(
    (c) => c.status === "sem_resposta",
  ).length;
  const reciclarDepois = contacts.filter(
    (c) => c.status === "reciclar_depois",
  ).length;
  ...
  return {
    today: {
      ...
      meetings: meetingsThisWeek.filter((m) => isToday(new Date(m.dueDate!)))
        .length,
    ...
    needFollowup: needFollowup.map((c) => ({
      ...
      daysSince: Math.floor(
        (now.getTime() - new Date(c.ultimaInteracao!).getTime()) / DAY_MS,
      ),
    })),
  };
}
...
export async function getPosVendaDashboard(userId: string) {
  ...
  const activeClients = clientes.filter(
    (c) => c.ultimaInteracao && new Date(c.ultimaInteracao) >= thirtyDaysAgo,
  );

  // Open pendencias (any field with content indicating pending items)
  const openPendencias = clientes.filter(
    (c) =>
  ...
  const expansionCandidates = clientes.filter(
    (c) => c.ultimaInteracao && new Date(c.ultimaInteracao) < thirtyDaysAgo,
  );
  ...
  const byResponsavel = new Map<
    string,
    { count: number; deals: number; value: number }
  >();
  ...
  const recentWins = deals
    .filter(
      (d) =>
        ...
        d.wonAt &&
        new Date(d.wonAt) >= new Date(now.getTime() - 90 * DAY_MS),
    )
  ...
  return {
    ...
    byResponsavel: [...byResponsavel.entries()]
      .map(([name, v]) => ({ name, ...v }))
      .sort((a, b) => b.value - a.value),
