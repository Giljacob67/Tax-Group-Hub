  ...
  try {
    const res = await fetch("https://openrouter.ai/api/v1/models", {
      headers: {
        ...
        "HTTP-Referer": "https://tax-group-hub.vercel.app",
        "X-Title": "Tax Group AI Hub",
      },
    });
    if (!res.ok) {
      ...
      return {
        success: false,
        models: [],
        error: `OpenRouter ${res.status}: ${err.slice(0, 200)}`,
      };
    }
      ...
      return {
        ...
        supportsTools:
          String(m.description || "")
            .toLowerCase()
            .includes("tool") ||
          String(m.id).includes("gpt-4") ||
          String(m.id).includes("claude"),
        supportsJson: true,
        priceInput: pricing.prompt
          ? `$${(Number(pricing.prompt) * 1_000_000).toFixed(2)} / 1M tokens`
          : undefined,
        priceOutput: pricing.completion
          ? `$${(Number(pricing.completion) * 1_000_000).toFixed(2)} / 1M tokens`
          : undefined,
        providerMetadata: m,
      };
    });
...
export async function discoverOllama(
  ...
  try {
    ...
    const tagsEndpoint = url.endsWith("/api")
      ? `${url}/tags`
      : `${url}/api/tags`;
    ...
    if (!res.ok) {
      const hint =
        res.status === 404 ? ` — Endpoint não encontrado: ${tagsEndpoint}` : "";
      return {
        success: false,
        models: [],
        error: `Ollama ${res.status}${hint}`,
      };
    }
    const data = (await res.json()) as {
      models?: Array<{
    ...
    if (err.name === "TimeoutError" || err.message?.includes("abort")) {
      return {
        success: false,
        models: [],
        error: "Timeout: Ollama não respondeu em 8s.",
      };
    }
    return { success: false, models: [], error: err.message };
  }
  ...
  try {
    ...
    const models = chatModels.map((m): DiscoveredModel => {
      ...
      return {
        ...
        contextWindow: cw,
        supportsVision:
          id.includes("4o") || id.includes("vision") || id.includes("gpt-5"),
        supportsTools:
          id.includes("gpt-4") ||
          id.includes("gpt-5") ||
          id.includes("o1") ||
  ...
  try {
    ...
    if (!ping.ok && ping.status !== 400) {
      ...
      return {
        success: false,
        models: [],
        error: `Anthropic ${ping.status}: ${err.slice(0, 200)}`,
      };
    }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        success: false,
        models: [],
        error: `Google ${res.status}: ${err.slice(0, 200)}`,
      };
    }
    const data = (await res.json()) as {
      models?: Array<{
    ...
    const models = (data.models || [])
      ...
      .map((m): DiscoveredModel => {
        ...
        return {
          ...
          supportsTools: (m.supportedGenerationMethods || []).includes(
            "generateContent",
          ),
          supportsJson: true,
...
export async function discoverCustomOpenAI(
  baseUrl: string,
  apiKey?: string,
): Promise<DiscoveryResult> {
  try {
    ...
    if (!res.ok) {
      return {
        success: false,
        models: [],
        error: `Custom provider ${res.status}`,
      };
    }
    const data = (await res.json()) as {
  ...
  switch (provider) {
    ...
    default:
      return {
        success: false,
        models: [],
        error: `Provider '${provider}' not supported for discovery.`,
      };
  }
}
