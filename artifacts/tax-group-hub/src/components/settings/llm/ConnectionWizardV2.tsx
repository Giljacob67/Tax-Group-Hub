...
const USAGE_OPTIONS = [
  {
    ...
    icon: MessageSquare,
    desc: "Interação natural com leads e clientes",
  },
  {
    id: "reasoning",
    label: "Diagnóstico Tributário",
    icon: Sparkles,
    desc: "Análise técnica profunda de cenários fiscais",
  },
  {
    id: "fast",
    label: "RAG / Base de Conhecimento",
    icon: BookOpen,
    desc: "Recuperação de informações documentais",
  },
  {
    id: "json",
    label: "Automações JSON",
    icon: Code,
    desc: "Saída estruturada para integrações",
  },
  {
    id: "vision",
    label: "Baixo Custo",
    icon: DollarSign,
    desc: "Modelo econômico para alto volume",
  },
  {
    id: "embedding",
...
export function ConnectionWizardV2({
  providers,
  staticModels,
  initialProviderId,
  onClose,
  onCreated,
}: ConnectionWizardV2Props) {
  ...
  const canAdvance = () => {
    ...
    if (step === 3)
      return (
        manualMode ||
        (validationResults && validationResults.every((r) => r.ok))
      );
      ...
      setValidationResults([
        {
          ...
          latencyMs: 0,
          message: err.message,
          userMessage: "Erro de rede ao validar credenciais.",
        },
      ]);
    } finally {
      setValidating(false);
  ...
  const filteredModels = discoveredModels.filter(
    (m) =>
      !modelSearch ||
  ...
  const staticForProvider = (staticModels || []).filter(
    (m) => !selectedProvider || m.provider === selectedProvider.id,
  );
  const filteredStatic = staticForProvider.filter(
    (m) =>
      !modelSearch ||
      m.name.toLowerCase().includes(modelSearch.toLowerCase()) ||
      m.id.toLowerCase().includes(modelSearch.toLowerCase()),
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-border/50 flex items-center gap-1 overflow-x-auto">
          {(
            ...
            <div
              key={s.id}
              className={`flex items-center gap-1 text-[11px] font-medium px-2 py-1 rounded-full ${step === s.id ? "bg-primary/10 text-primary" : step > s.id ? "text-muted-foreground" : "text-muted-foreground/40"}`}
            >
              {step > s.id ? (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <AnimatePresence mode="wait">
            ...
            {step === 2 && selectedProvider && (
              ...
              >
                <p className="text-sm text-muted-foreground">
                  Insira as credenciais para {selectedProvider.name}.
                </p>
                ...
                {manualMode ? (
                  <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      A validação será pulada. Use com cuidado.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    {validationResults && (
                      <div className="space-y-2">
                        {validationResults.map((r, i) => (
                          <div
                            key={i}
                            className={`rounded-lg border p-3 ${r.ok ? "border-emerald-500/20 bg-emerald-500/5" : "border-red-500/20 bg-red-500/5"}`}
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-sm font-medium ${r.ok ? "text-emerald-400" : "text-red-400"}`}
                              >
                                {r.message}
                              </span>
                            </div>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 mt-1 flex-wrap">
                            ...
                            {m.supportsTools && (
                              <Badge
                                variant="outline"
                                className="text-[10px] h-4 border-emerald-500/20 text-emerald-400"
                              >
                                Tools
                              </Badge>
                            )}
                            {m.supportsJson && (
                              <Badge
                                variant="outline"
                                className="text-[10px] h-4 border-blue-500/20 text-blue-400"
                              >
                                JSON
                              </Badge>
                            )}
                    ...
                    {filteredStatic.length > 0 ? (
                      <>
                        ...
                        <p className="text-[10px] text-muted-foreground/60 pt-2 border-t border-border/40">
                          Ou clique em "Buscar modelos" acima para descobrir via
                          API do provedor (requer chave válida).
                        </p>
                      </>
                    ) : (
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Nenhum modelo pré-cadastrado para{" "}
            ...
            {step === 5 && (
              ...
              >
                <p className="text-sm text-muted-foreground">
                  Qual a finalidade principal desta conexão?
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {USAGE_OPTIONS.map((u) => (
                    ...
                    >
                      <u.icon
                        className={`w-4 h-4 mt-0.5 ${usageType === u.id ? "text-primary" : "text-muted-foreground"}`}
                      />
                      <div>
                        <p className="text-sm font-medium text-foreground">
                          {u.label}
              ...
              >
                {manualMode ? (
                  <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 text-center">
                    <AlertCircle className="w-6 h-6 text-amber-400 mx-auto mb-2" />
                    <p className="text-sm text-foreground font-medium">
                      Modo manual — teste pulado
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      A conexão será salva sem teste real.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    {testResult && (
                      <div
                        className={`rounded-lg border p-3 ${testResult.ok ? "border-emerald-500/20 bg-emerald-500/5" : "border-red-500/20 bg-red-500/5"}`}
                      >
                        <div className="flex items-center gap-2">
                          {testResult.ok ? (
                            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                          ...
                          <span
                            className={`text-sm font-medium ${testResult.ok ? "text-emerald-400" : "text-red-400"}`}
                          >
                            {testResult.message}
                          </span>
                        </div>
              ...
              >
                <div className="rounded-xl border border-border/50 bg-card/40 p-4 space-y-3">
                  ...
                  <div className="rounded-lg bg-muted/20 p-2.5 text-xs text-muted-foreground border border-border/30">
                    🔒 Chave API armazenada com criptografia AES-256-GCM. Nunca
                    exposta no frontend.
                  </div>
                </div>
