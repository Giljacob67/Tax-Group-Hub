...
import { Activity, CheckCircle2, AlertCircle, Clock, X, Loader2, Minus } from "lucide-react";
...
const STAGE_ORDER: DiagnosticStage[] = ["auth", "models", "chat", "json", "tools"];
...
export function HealthCheckPanel({ results, loading, onRun, onClose }: HealthCheckPanelProps) {
  const [expandedId, setExpandedId] = useState<number | null>(null);

  const okCount = results.filter((r) => !r.error && r.diagnostics?.overall === "ok").length;
    ...
    >
      <div className="p-4 border-b border-border/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground">Health Check</h3>
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" className="h-7 text-xs gap-1" onClick={onRun} disabled={loading}>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-muted-foreground border-b border-border/30">
                    <th className="text-left px-2 py-1.5 font-medium">Conexão</th>
                    {STAGE_ORDER.map((s) => (
                      <th key={s} className="text-center px-2 py-1.5 font-medium w-16">{STAGE_LABELS[s]}</th>
                    ))}
                ...
                <tbody>
                  {results.map((r) => {
                    ...
                    const stageMap = new Map(r.diagnostics?.results.map((res) => [res.stage, res.ok]) || []);
                    ...
                    return (
                      <>
                        <tr
                          ...
                          onClick={() => setExpandedId(isExpanded ? null : r.connectionId)}
                        >
                            ...
                            <td colSpan={8} className="px-2 py-2">
                              <div className="rounded-lg bg-muted/20 p-2 space-y-1">
                                {r.diagnostics.results.map((res) => (
                                  <div key={res.stage} className="flex items-center gap-2 text-xs">
                                    <StageDot ok={res.ok} />
                                    <span className="text-muted-foreground">{STAGE_LABELS[res.stage]}:</span>
                                    <span className={res.ok ? "text-emerald-400" : "text-red-400"}>{res.message}</span>
                                    <span className="text-muted-foreground/60 ml-auto">{res.latencyMs}ms</span>
                                  </div>
                                ))}
                              </div>
                            </td>
