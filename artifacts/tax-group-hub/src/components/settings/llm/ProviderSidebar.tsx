...
export default function ProviderSidebar({
  ...
  const counts = (pid: string) => connections.filter((c) => c.provider === pid).length;
  ...
  return (
    <div className="w-60 flex-shrink-0 border-r border-border/30 bg-background/50 flex flex-col h-full">
      <div className="p-4 border-b border-border/30">
        ...
        <Button size="sm" className="w-full text-xs gap-1.5" onClick={onNewConnection}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        <button
          onClick={() => { setSelected(null); onSelectProvider(null); }}
        ...
        {providers.map((p) => {
          ...
          return (
            <button
              key={p.id}
              onClick={() => { setSelected(p.id); onSelectProvider(p); }}
              className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-all text-left ${
                isSel
