...
function RoleBadge({ role, isActive }: { role: string; isActive?: boolean }) {
  ...
  return (
    <Badge
      variant="outline"
      className={`${cfg.color} text-[10px] ${!isActive ? "opacity-50 line-through" : ""}`}
    >
      {cfg.label}
    </Badge>
  );
...
function GrantRoleDialog({
  ...
  const grantMutation = useGrantCrmUserRole({
    mutation: {
      ...
      onError: () =>
        toast({ title: "Erro ao atribuir papel", variant: "destructive" }),
    },
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="role-scope" className="text-xs">
              Escopo (opcional)
            </Label>
            <Input
              id="role-scope"
        ...
        <DialogFooter>
          ...
          <Button
            onClick={submit}
            disabled={!selectedRole || grantMutation.isPending}
          >
        ...
        <td className="p-3">
          <div className="flex flex-wrap gap-1">
            {activeRoles.length === 0 ? (
              ...
              activeRoles.map((r) => (
                <RoleBadge key={r.id} role={r.role} isActive />
              ))
            )}
            {inactiveRoles.length > 0 && (
              <span className="text-[10px] text-muted-foreground ml-1">
                (+{inactiveRoles.length} inativo
                {inactiveRoles.length !== 1 ? "s" : ""})
              </span>
            )}
          </div>
        </td>
                ...
                <tbody>
                  {user.roles.map((r) => (
                    <tr key={r.id} className="border-t border-border/30">
                      <td className="p-1.5">
                        <RoleBadge role={r.role} isActive={r.isActive} />
                      </td>
                      ...
                      <td className="p-1.5">
                        {r.isActive ? (
                          ...
                          <Badge
                            variant="outline"
                            className="text-[10px] text-slate-400 border-slate-500/30"
                          >
                            Inativo
                          </Badge>
                        )}
                      ...
                      <td className="p-1.5 text-right">
                        {r.isActive && (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-6 px-2 text-[11px] text-muted-foreground hover:text-red-400"
                            onClick={() =>
      ...
      <Card className="border-border/50 bg-card/50 h-full">
        <CardContent className="p-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
            Apenas administradores podem gerenciar papéis de usuários. Seu papel
            atual:{" "}
      ...
      <CardHeader className="pb-3 border-b border-border/50 flex flex-row items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Defina quem pode fazer o quê no CRM. {users.length} usuário
            {users.length !== 1 ? "s" : ""} · {roles.length} papéis disponíveis.
          </p>
        </div>
        <Button
          variant="outline"
