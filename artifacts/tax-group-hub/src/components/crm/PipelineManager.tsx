...
function getStageLabel(stage: string): string {
  return (
    (PIPELINE_STAGE_LABELS as Record<string, string | undefined>)[stage] ||
    stage
  );
}
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">
        Etapas do funil{" "}
        <span className="normal-case">(arraste para reordenar)</span>
      </p>
        ...
        <AnimatePresence>
          {stages.map((stage, idx) => (
            ...
            >
              ...
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${getStageColor(stage)}`}
              >
                {getStageLabel(stage)}
              </span>
      ...
      {/* Suggested stages */}
      <div>
        <p className="text-[11px] text-muted-foreground/60 uppercase tracking-wider mb-1.5">
          Sugestões (Pipeline Tax Group)
        </p>
    ...
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getListCrmPipelinesQueryKey(),
        });
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-2.5 flex items-start gap-2">
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          O funil operacional padrão é o{" "}
          <strong className="text-foreground">Pipeline Tax Group</strong> (16
          etapas). Use este formulário apenas se precisar criar um funil
          customizado para uso específico.
        </p>
      </div>
    ...
    <div className="w-[380px] bg-card border border-border/50 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="p-2 space-y-1 max-h-[400px] overflow-y-auto">
        {isLoading ? (
          ...
          allPipelines.map((pl) => {
            const isActive =
              activePipelineId === (pl.id === 0 ? "default" : String(pl.id));
            return (
              <div
                key={pl.id}
                className={`group flex items-center gap-2.5 p-3 rounded-xl border transition-all cursor-pointer ${
                  isActive
                    ...
                    : "bg-card/50 border-border/40 hover:border-border/70 hover:bg-muted/20"
                }`}
                onClick={() =>
                  onSelect(pl.id === 0 ? "default" : String(pl.id))
                }
              >
                {/* Active indicator */}
                <div
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? "bg-primary" : "bg-muted"}`}
                />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {isActive && (
                      <Badge
                        variant="outline"
                        className="text-[11px] border-primary/40 text-primary px-1.5 py-0 h-4"
                      >
                        Ativo
                      </Badge>
                    )}
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Can permission="canEditPipeline">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          deletePipeline.mutate({ id: pl.id });
                        }}
                ...
                <ChevronRight
                  className={`w-3.5 h-3.5 flex-shrink-0 transition-colors ${isActive ? "text-primary" : "text-muted-foreground/30"}`}
                />
              </div>
            );
          })
      ...
      <div className="px-4 py-2.5 border-t border-border/30 bg-muted/10">
        <p className="text-xs text-muted-foreground">
          Clique em um funil para ativá-lo no Kanban. O funil ativo define as
          colunas exibidas.
        </p>
      </div>
    </div>
