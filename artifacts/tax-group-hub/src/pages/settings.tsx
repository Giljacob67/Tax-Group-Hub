...
function WhatsAppSection() {
  ...
  const { data: chData, isLoading: chLoading } = useListChannels({ query: { queryKey: ["/api/settings/channels"] } } as any);
  const { data: agData } = useListAgents({ query: { queryKey: ["/api/agents"] } } as any);
  ...
  const createChannel = useCreateChannel({
    mutation: {
      onSuccess: (data) => {
        setWebhookUrl(`${window.location.origin}/api/webhooks/whatsapp/${data.channel.id}`);
      ...
      onError: () => toast({ title: "Erro ao salvar canal", variant: "destructive" }),
    },
  });
  ...
  const deleteChannel = useDeleteChannel({
    mutation: {
      ...
      onError: () => toast({ title: "Erro ao remover canal", variant: "destructive" }),
    },
  });
          ...
          {channels.map((ch: { id: number; agentId: string }) => {
            ...
            return (
              <div key={ch.id} className="flex items-center gap-3 bg-card/50 border border-border/40 rounded-xl px-4 py-3">
                ...
                >
                  {deleteChannel.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                </Button>
              </div>
            );
          })}
        ...
        {showForm && (
          ...
          >
            <div className="bg-card/50 border border-border/40 rounded-xl p-5 space-y-4">
              ...
              <Button onClick={handleSave} disabled={createChannel.isPending} size="sm">
                {createChannel.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1" /> : <Save className="w-3.5 h-3.5 mr-1" />}
                Salvar Canal
              </Button>
...
function BrandingSection() {
  ...
  const { data: brandingData, isLoading } = useGetBrandingConfig({ query: { queryKey: ["/api/branding/config"] } } as any);
  ...
  const updateBranding = useUpdateBrandingConfig({
    mutation: {
      onSuccess: () => {
        toast({ title: "Identidade salva! Recarregando..." });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            ...
            <Input value={displayValues.companyName} onChange={e => { setFormValues(p => ({ ...p, companyName: e.target.value })); setDirty(true); }} />
          </div>
          <div>
            ...
            <div className="flex gap-2 items-center">
              <input
                ...
                onChange={e => { setFormValues(p => ({ ...p, primaryColor: e.target.value })); setDirty(true); }}
          ...
          <div>
            ...
            <Input
              ...
              onChange={e => { setFormValues(p => ({ ...p, customDomain: e.target.value })); setDirty(true); }}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center gap-4 border-2 border-dashed border-border/30 rounded-2xl p-6 bg-muted/10">
          {displayValues.logoUrl
            ? <img src={displayValues.logoUrl} className="h-16 object-contain" loading="lazy" alt="Logo" />
      ...
      <div className="pt-4 border-t border-border/30 flex justify-end">
        <Button onClick={handleSave} disabled={updateBranding.isPending}>
          {updateBranding.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Save className="w-4 h-4 mr-2" />}
          Salvar Identidade
        </Button>
      </div>
    </div>
