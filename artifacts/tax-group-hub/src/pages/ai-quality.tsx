...
import {
  ...
  FlaskConical, Plus, Play, Trash2, ChevronDown, ChevronUp,
  FileText, CheckCircle, XCircle, Clock, Loader2,
  BarChart3, AlertTriangle, Bot, RefreshCw,
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full border ${map[status] || map.pending}`}>
...
export default function AiQualityPage() {
  ...
  const handleCreateTest = async () => {
    if (!newTest.name.trim() || !newTest.agentId || !newTest.question.trim()) {
      toast({ title: "Preencha nome, agente e pergunta", variant: "destructive" });
      return;
    }
    try {
      ...
      <div className="border-b border-border bg-card/50 px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-foreground">Qualidade & Rastreabilidade</h1>
              <p className="text-xs text-muted-foreground">Avaliação e monitoramento das respostas dos agentes IA</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-6 max-w-4xl">
            {loadingSummary ? (
              ...
              <>
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  <h2 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
                    <ThumbsUp className="w-4 h-4 text-primary" /> Feedback dos Usuários
                  </h2>
                  {summary.totalFeedback === 0 ? (
                    <div className="text-center py-6 text-muted-foreground text-sm">
                      <AlertTriangle className="w-8 h-8 mx-auto mb-2 opacity-30" />
                      Nenhum feedback coletado ainda. Os usuários podem avaliar respostas individuais no chat.
                    </div>
                  ) : (
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2 text-emerald-400">
                        <ThumbsUp className="w-5 h-5" />
                        <span className="text-2xl font-bold">{summary.positiveFeedback}</span>
                        <span className="text-xs text-muted-foreground">positivos</span>
                      </div>
                      <div className="flex items-center gap-2 text-rose-400">
                        <ThumbsDown className="w-5 h-5" />
                        <span className="text-2xl font-bold">{summary.negativeFeedback}</span>
                        <span className="text-xs text-muted-foreground">negativos</span>
                      </div>
                      {summary.satisfactionRate !== null && (
                        <div className="ml-auto text-center">
                          <div className="text-3xl font-bold text-primary">{summary.satisfactionRate}%</div>
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  <h2 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-primary" /> Guardrails Ativos
                  </h2>
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <div className="flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                      Fontes RAG visíveis por mensagem — usuário sabe de onde vem a informação
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0" />
              ...
              <div className="text-center text-muted-foreground py-12">Erro ao carregar métricas</div>
            )}
          </div>
        )}
          ...
          <div className="max-w-5xl space-y-4">
            ...
            {loadingRuns ? (
              ...
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p>Nenhuma execução ainda. Use os agentes no chat para gerar logs.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-card border-b border-border">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Agente</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Modelo</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tokens</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Latência</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {runs.map(run => (
                      <tr key={run.id} className="hover:bg-card/50 transition-colors">
                        <td className="px-4 py-3 text-xs text-muted-foreground">
                          {format(new Date(run.createdAt), "dd/MM HH:mm", { locale: ptBR })}
                        </td>
                        <td className="px-4 py-3 text-xs">
                          <span className="flex items-center gap-1.5">
                            <Bot className="w-3 h-3 text-primary/60 flex-shrink-0" />
                            <span className="truncate max-w-[120px]">{run.agentId || "—"}</span>
                          </span>
                        </td>
                        <td className="px-4 py-3 text-xs text-foreground/70">{run.model || "—"}</td>
                        <td className="px-4 py-3 text-xs text-foreground/70">{run.totalTokens?.toLocaleString("pt-BR") || "—"}</td>
                        ...
                        <td className="px-4 py-3">
                          {run.success ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full border bg-rose-500/10 text-rose-400 border-rose-500/30" title={run.errorMessage || ""}>
        ...
        {activeTab === "tests" && (
          <div className="max-w-3xl space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-foreground">Casos de Teste por Agente</h2>
              <button
            ...
            {showNewTestForm && (
              <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                <h3 className="text-sm font-semibold text-foreground">Novo Caso de Teste</h3>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setNewTest(p => ({ ...p, agentId: e.target.value }))}
                      className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary transition-all"
                    >
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={e => setNewTest(p => ({ ...p, question: e.target.value }))}
                  ...
                  <div className="col-span-2">
                    <label className="text-xs text-muted-foreground mb-1 block">Resposta esperada (opcional)</label>
                    <textarea
                      value={newTest.expectedAnswer}
                      onChange={e => setNewTest(p => ({ ...p, expectedAnswer: e.target.value }))}
                  ...
                  <div className="col-span-2">
                    <label className="text-xs text-muted-foreground mb-1 block">Critérios de avaliação (opcional)</label>
                    <input
                      value={newTest.criteria}
                      onChange={e => setNewTest(p => ({ ...p, criteria: e.target.value }))}
            ...
            {loadingTests ? (
              ...
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p>Nenhum caso de teste criado. Crie um para validar o comportamento dos agentes.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {testCases.map(tc => {
                  ...
                  return (
                    <div key={tc.id} className="bg-card border border-border rounded-xl overflow-hidden">
                      <div className="p-4 flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-semibold text-foreground truncate">{tc.name}</span>
                            {latestRun && <StatusBadge status={latestRun.status} />}
                          </div>
                          <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                            <Bot className="w-3 h-3 flex-shrink-0" /> {tc.agentId}
                          </div>
                          <div className="text-xs text-foreground/70 line-clamp-2">{tc.question}</div>
                          {tc.criteria && (
                            <div className="text-[11px] text-muted-foreground mt-1 italic">Critérios: {tc.criteria}</div>
                          )}
                        </div>
                      ...
                      {isExpanded && (
                        <div className="border-t border-border p-4 bg-background/50">
                          ...
                          {runs.length === 0 ? (
                            <p className="text-xs text-muted-foreground">Nenhuma execução. Clique em Executar ▶ para rodar.</p>
                          ) : (
                            <div className="space-y-3">
                              {runs.map(run => (
                                <div key={run.id} className="border border-border rounded-lg p-3 bg-card">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
                                      ...
                                      {run.tokensUsed != null && <span>{run.tokensUsed} tokens</span>}
                                      <span>{format(new Date(run.createdAt), "dd/MM HH:mm", { locale: ptBR })}</span>
                                    </div>
                                  </div>
                                  {run.ragSources && run.ragSources.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mb-2">
                                      {run.ragSources.map((src, i) => (
                                        <span key={i} className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20">
                                  ...
                                  {run.notes && (
                                    <div className="text-[11px] text-destructive mt-1">{run.notes}</div>
                                  )}
                                </div>
                              ))}
                            </div>
