...
export default function AgentChat() {
  ...
  const [orchestrationPlan, setOrchestrationPlan] = useState<Array<{
    agentId: string;
    task: string;
  }> | null>(null);
  ...
  const { data: conversations, isLoading: isLoadingConvs } =
    useListConversations({ agentId });
  const { data: activeConv, isLoading: isLoadingMessages } = useGetConversation(
    activeConvId!,
    {
  ...
  const availableConnections: LlmConnection[] =
    (llmConnectionsData?.connections ?? []) as unknown as LlmConnection[];

  useEffect(() => {
    if (activeConv?.connectionId && availableConnections.length) {
      const match = availableConnections.find(
        (c) => c.id === activeConv.connectionId,
      );
    ...
    try {
      ...
      if (!convId) {
        const newConv = await createMutation.mutateAsync({
          data: {
            ...
            connectionId: selectedConnection?.id || undefined,
          },
        });
        convId = newConv.id;
        setIsCreatingNewChat(false);
        ...
        queryClient.invalidateQueries({
          queryKey: getListConversationsQueryKey({ agentId }),
        });
        queryClient.invalidateQueries({
          queryKey: getListConversationsQueryKey({ agentId }),
        });
      ...
      const response = await fetch(`/api/conversations/${convId}/messages`, {
        ...
        body: JSON.stringify({
          ...
          connectionId:
            selectedConnection?.id || activeConv?.connectionId || undefined,
          stream: true,
        }),
      });
          ...
          for (const line of lines) {
            if (line.startsWith("data: ")) {
              try {
                ...
                /* SSE parse error — silently skip malformed chunks */
              }
            }
          }
        }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: exportedData } = useExportConversation(activeConvId || "", {
  ...
  const handleRegenerate = async (msgId: string) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: getGetConversationQueryKey(activeConvId),
      });
  ...
  const handleSaveSystemPrompt = () => {
    ...
    if (agentId) {
      try {
        if (newPrompt)
          localStorage.setItem(`taxgroup_system_prompt_${agentId}`, newPrompt);
  ...
  function parseOrchestrationPlan(
    content: string,
  ): Array<{ agentId: string; task: string }> | null {
    const match = content.match(
      /\[ORCHESTRATION_PLAN\]\s*([\s\S]*?)\s*\[\/ORCHESTRATION_PLAN\]/,
    );
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-3 p-8">
        ...
        <p className="text-xs text-center max-w-sm">
          Verifique se o agente está configurado corretamente ou retorne para a
          lista de agentes.
        </p>
      </div>
    );
          ...
          {isLoadingConvs ? (
            ...
            <div className="text-center p-6">
              ...
              <p className="text-sm text-muted-foreground">
                {searchFilter
                  ? "Nenhum resultado"
                  : "Inicie uma conversa com o agente para transformar uma oportunidade em plano de ação."}
              </p>
            </div>
          ) : (
            filteredConversations?.map((conv) => (
              ...
              >
                <div className="flex items-start space-x-3 overflow-hidden flex-1 min-w-0">
                  <MessageSquare
                    className={`w-4 h-4 mt-0.5 flex-shrink-0 ${activeConvId === conv.id ? "text-primary" : "text-muted-foreground"}`}
                  />
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground border border-border">
                  {agent.blockLabel}
                </span>
                <span
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${isApiOnline ? "bg-primary" : "bg-muted-foreground"}`}
                  />
          ...
          <div className="max-w-3xl mx-auto space-y-6 pb-4">
              ...
              >
                <div className="bg-card border border-border rounded-2xl p-8 max-w-lg w-full text-center mb-6">
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed">
                    {agent.description ||
                      "Assistente especializado da Tax Group."}
                  </p>
                  {isDemo && (
                ...
                {(agent.suggestedPrompts?.length > 0 || isDemo) && (
                  <div className="w-full max-w-2xl">
                    <p className="text-xs text-muted-foreground uppercase tracking-widest font-semibold mb-3 text-center">
                      Sugestões de início
                    </p>
            ...
            <AnimatePresence>
              {activeConv?.messages?.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[92%] md:max-w-[85%] flex ${msg.role === "user" ? "flex-row-reverse" : "flex-row"} items-end gap-3`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mb-1 text-base select-none ${msg.role === "user" ? "bg-secondary" : "bg-primary/15 border border-primary/20"}`}
                    >
                      {msg.role === "user" ? (
                        <User className="w-4 h-4 text-foreground/70" />
                      ) : agent.icon ? (
                        <span role="img" aria-label="Assistente">
                          {agent.icon}
                        </span>
                      ) : (
                    ...
                    <div
                      className={`relative group p-4 rounded-2xl ${msg.role === "user" ? "bg-primary text-white rounded-br-sm shadow-md" : "bg-card text-foreground rounded-bl-sm shadow-sm border border-border"}`}
                    >
                      {msg.role === "assistant" && (
                        <button
                          onClick={() => handleCopy(msg.content, msg.id)}
                          className="absolute right-2 top-2 md:-right-10 md:top-2 p-1.5 rounded-md text-muted-foreground hover:bg-secondary hover:text-foreground opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity"
                          title="Copiar"
                      ...
                      <div
                        className={`text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none ${msg.role === "user" ? "prose-p:text-white prose-strong:text-white" : ""}`}
                      >
                        <ReactMarkdown>
                          {msg.role === "assistant"
                            ? stripOrchestrationBlock(msg.content)
                            : msg.content}
                        </ReactMarkdown>
                      </div>
                      <div
                        className={`text-xs mt-2 text-right ${msg.role === "user" ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                      >
                        {(() => {
                          try {
                      ...
                      {msg.role === "assistant" &&
                        ...
                        (() => {
                          ...
                          return (
                            ...
                            >
                              <Rocket className="w-4 h-4" /> Executar Plano com
                              Agentes ({plan.length} agente
                              {plan.length !== 1 ? "s" : ""})
                            </button>
                          );
                        })()}
                          ...
                          return (
                            <div className="mt-3 border-t border-border/50 pt-2 space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full border ${confColors[confidenceLevel] || confColors.none}`}
                                  >
                              ...
                              {isOpen && ragSources?.length > 0 && (
                                <div className="space-y-1">
                                  {ragSources.map(
                                    (
                                      src: { filename: string; score: number },
                                      i: number,
                                    ) => (
                                      <div
                                        key={i}
                                        className="flex items-center justify-between text-[10px] bg-muted/40 rounded px-2 py-1 gap-2"
                                      >
                                        ...
                                        <span
                                          className={`flex-shrink-0 font-medium ${src.score >= 0.75 ? "text-emerald-400" : src.score >= 0.5 ? "text-amber-400" : "text-orange-400"}`}
                                        >
                    ...
                    <div className="relative p-4 rounded-2xl bg-card/30 text-foreground rounded-bl-sm shadow-sm">
                      {streamingContent ? (
                        <div className="text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none">
                          <ReactMarkdown>
                            {stripOrchestrationBlock(streamingContent)}
                          </ReactMarkdown>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div>
            ...
            <p className="text-xs text-foreground leading-relaxed">
              {agent.description || "Assistente especializado da Tax Group."}
            </p>
          </div>
          {activeConv && (
            <div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              <button
                onClick={() =>
                  handleCreateAndSend(
                    "Gere um resumo executivo das oportunidades tributárias identificadas.",
                  )
                }
              ...
              <button
                onClick={() =>
                  handleCreateAndSend(
                    "Baseado nesta conversa, gere uma proposta comercial estruturada.",
                  )
                }
                className="w-full text-left px-3 py-2 rounded-lg text-xs bg-background border border-border hover:border-primary/30 hover:bg-muted transition-colors flex items-center gap-2"
              >
                <FileText className="w-3.5 h-3.5 text-muted-foreground" /> Gerar
                proposta
              </button>
              <button
                onClick={() =>
                  handleCreateAndSend(
                    "Crie uma tarefa de follow-up com os próximos passos.",
                  )
                }
                className="w-full text-left px-3 py-2 rounded-lg text-xs bg-background border border-border hover:border-primary/30 hover:bg-muted transition-colors flex items-center gap-2"
              >
        ...
        {showMobileHistory && (
          <>
            ...
            >
              ...
              <div
                className="overflow-y-auto px-3 pb-6 space-y-1.5"
              ...
              >
                {filteredConversations?.map((conv) => (
                  ...
                  >
                    <MessageSquare
                      className={`w-4 h-4 flex-shrink-0 ${activeConvId === conv.id ? "text-primary" : "text-muted-foreground"}`}
                    />
