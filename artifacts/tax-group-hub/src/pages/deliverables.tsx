...
const TYPES = [
  {
    ...
    icon: Target,
    desc: "Análise tributária completa com hipóteses, riscos e recomendações.",
  },
  {
    id: "proposta",
    label: "Proposta Comercial",
    icon: FileText,
    desc: "Proposta estruturada com escopo, produto, metodologia e cronograma.",
  },
  {
    id: "resumo_oportunidade",
    label: "Resumo de Oportunidade",
    icon: Sparkles,
    desc: "Overview executivo com score, potencial e abordagem recomendada.",
  },
  {
    id: "followup",
    label: "Follow-up Comercial",
    icon: MessageSquare,
    desc: "Mensagem e próxima ação para manutenção do relacionamento.",
  },
  {
    id: "roteiro_reuniao",
    label: "Roteiro de Reunião",
    icon: CalendarDays,
    desc: "Pauta, perguntas-chave, objeções e fechamento sugerido.",
  },
];
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_COLORS[status] || STATUS_COLORS.draft}`}
    >
...
function ConfBadge({ level }: { level: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] ${CONF_COLORS[level] || CONF_COLORS.none}`}
    >
...
function DeliverableWizard({
  ...
  const [selectedContact, setSelectedContact] =
    useState<DeliverableRow["contactId"] extends infer T ? any : any>(null);
  ...
  const generateMutation = useGenerateDeliverable({
    mutation: {
      ...
      onError: (err: any) => {
        toast({
          title: err?.message || "Erro ao gerar",
          variant: "destructive",
        });
      },
    },
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                <p className="text-sm text-muted-foreground mb-4">
                  Selecione uma empresa/lead do CRM para vincular ao entregável
                  (opcional).
                </p>
                ...
                {loadingContacts ? (
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto">
                    {contacts.map((c: any) => (
                      ...
                      >
                        <Building2
                          className={`w-4 h-4 flex-shrink-0 ${selectedContact?.id === c.id ? "text-primary" : "text-muted-foreground"}`}
                        />
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-muted-foreground">
                            {c.cnpj} • {c.regimeTributario || "regime n/d"}{" "}
                            {c.aiScore ? `• Score: ${c.aiScore}` : ""}
                          </div>
                        </div>
                ...
                {selectedContact && (
                  <div className="mt-3 flex items-center gap-2 text-xs text-primary">
                    ...
                    {selectedContact.razaoSocial || selectedContact.cnpj}{" "}
                    selecionada
                  </div>
                )}
              </motion.div>
            ...
            {step === 2 && (
              ...
              >
                <p className="text-sm text-muted-foreground mb-4">
                  Qual tipo de entregável você precisa gerar?
                </p>
                <div className="space-y-2">
                  {TYPES.map((t) => (
                    ...
                    >
                      <t.icon
                        className={`w-5 h-5 mt-0.5 flex-shrink-0 ${selectedType === t.id ? "text-primary" : "text-muted-foreground"}`}
                      />
                      <div>
            ...
            {step === 3 && (
              ...
              >
                <p className="text-sm text-muted-foreground mb-4">
                  Qual produto ou contexto tributário é o foco?
                </p>
                <div className="grid grid-cols-2 gap-2 mb-4">
                  {PRODUCTS.map((p) => (
                    ...
                    >
                      ...
                      <span className={selectedProduct === p.id ? "" : "pl-5"}>
                        {p.label}
                      </span>
                    </button>
                  ))}
              ...
              >
                ...
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-xs text-amber-400 space-y-1">
                  <div className="font-semibold flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5" /> Guardrails ativos
                  </div>
                  <div>
                    O entregável será marcado como rascunho e exigirá revisão
                    humana antes do envio ao cliente.
                  </div>
                  <div>
                    Resultados tributários não são apresentados como garantia —
                    apenas hipóteses baseadas nas informações disponíveis.
                  </div>
                </div>
              </motion.div>
            )}
          ...
          {step < 4 ? (
            ...
            >
              {generating ? (
                ...
                <>
                  <Sparkles className="w-4 h-4" /> Gerar Entregável
                </>
              )}
            </button>
          )}
...
function DeliverableEditor({
          ...
          prev
            ? {
                ...
                sections: prev.sections.map((s) =>
                  s.id === updated.id ? { ...s, content: editingContent } : s,
                ),
              }
            : prev,
        );
        setActiveSection((prev) =>
          prev ? { ...prev, content: editingContent } : prev,
        );
  ...
  const setData = useCallback(
    (
      updater: (
        prev: DeliverableDetail | undefined,
      ) => DeliverableDetail | undefined,
    ) => {
      queryClient.setQueryData([`/api/deliverables/${id}`], (old: any) =>
        updater(old as DeliverableDetail | undefined),
      );
    },
    [queryClient, id],
  );
            ...
            {sections.map((s) => (
              ...
              >
                <div className="flex items-center justify-between gap-1">
                  ...
                  <span
                    className={`flex-shrink-0 ${CONF_COLORS[s.confidenceLevel || "none"]}`}
                  >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      editingContent === activeSection.content
                    }
          ...
          <div className="w-72 border-l border-border flex-shrink-0 overflow-y-auto bg-card/30">
            {showSources && (
              <div className="p-4">
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {s.similarityScore != null && (
                            <span
                              className={`text-[10px] font-semibold ml-1 flex-shrink-0 ${s.similarityScore >= 75 ? "text-emerald-400" : s.similarityScore >= 50 ? "text-amber-400" : "text-orange-400"}`}
                            >
                              {s.similarityScore}%
                            </span>
                          )}
                ...
                {deliverable.guardrailWarnings &&
                  deliverable.guardrailWarnings.length > 0 && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-2.5 mb-3">
                      {deliverable.guardrailWarnings.map(
                        (w: string, i: number) => (
...
export default function DeliverablesPage() {
  ...
  const deliverables = (listData?.deliverables ??
  ...
  const lowConf = deliverables.filter(
    (d) => d.confidenceLevel === "low" || d.confidenceLevel === "none",
  ).length;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Gere diagnósticos, propostas e materiais de follow-up com fontes
                confiáveis da Tax Group.
              </p>
            </div>
          </div>
          <button
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20 text-muted-foreground">
                ...
                <p className="text-xs mt-1">
                  Clique em "Novo Entregável" para gerar o primeiro.
                </p>
                <button
                  onClick={() => setShowWizard(true)}
              ...
              <table className="w-full text-sm" aria-label="Lista de entregáveis">
                <caption className="sr-only">Lista de entregáveis</caption>
                  ...
                  {filtered.map((d) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <div className="text-[11px] text-muted-foreground">
                          {TYPES.find((t) => t.id === d.type)?.label || d.type}
                        </div>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-xs text-foreground/70 flex items-center gap-1">
                          {d.companyName ? (
                            <>
                              <Building2 className="w-3 h-3" /> {d.companyName}
                            </>
                          ) : (
                      ...
                      <td className="px-4 py-3 hidden xl:table-cell">
                        ...
                        {d.ragSourceCount != null && d.ragSourceCount > 0 && (
                          <span className="text-[10px] text-muted-foreground ml-1">
                            ({d.ragSourceCount} fontes)
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-xs text-muted-foreground hidden xl:table-cell">
                        {format(new Date(d.updatedAt), "dd/MM HH:mm", {
                          locale: ptBR,
                        })}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                            title="Exportar HTML"
                            aria-label="Exportar entregável como HTML"
                          >
                            <Download className="w-3.5 h-3.5" />
                          </button>
                          {d.status !== "archived" && (
                            <button
                              onClick={() =>
                                handleStatusChange(d.id, "archived")
                              }
                              className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                              title="Arquivar"
                              aria-label="Arquivar entregável"
                            >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. O entregável e todo seu histórico
