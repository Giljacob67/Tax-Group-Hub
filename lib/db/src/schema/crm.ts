...
export const crmContactsTable = pgTable(
  "crm_contacts",
  {
    ...
    influenciadores:
      jsonb("influenciadores").$type<
        Array<{ nome: string; cargo?: string; influencia?: string }>
      >(),
    socios:
      jsonb("socios").$type<
        Array<{ nome: string; cpf?: string; participacao?: string }>
      >(),
    ...
    proximoPassoRecomendado: jsonb("proximo_passo_recomendado").$type<
      Record<string, any>
    >(), // Phase 3
...
export const insertCrmPipelineSchema = createInsertSchema(
  crmPipelinesTable,
...
export const crmActivitiesTable = pgTable("crm_activities", {
  ...
  contactId: integer("contact_id")
    .notNull()
    .references(() => crmContactsTable.id, { onDelete: "cascade" }),
  dealId: integer("deal_id").references(() => crmDealsTable.id, {
    onDelete: "set null",
  }),
  userId: text("user_id").notNull(),
  orgId: integer("org_id"),
...
export const insertCrmEnrichmentLogSchema = createInsertSchema(
  crmEnrichmentLogTable,
).omit({ id: true, createdAt: true });
export type CrmEnrichmentLog = typeof crmEnrichmentLogTable.$inferSelect;
export type InsertCrmEnrichmentLog = z.infer<
  typeof insertCrmEnrichmentLogSchema
>;
...
export const insertCrmAttachmentSchema = createInsertSchema(
  crmAttachmentsTable,
).omit({ id: true, createdAt: true });
...
export const crmQualificationHistoryTable = pgTable(
...
export const insertCrmQualificationHistorySchema = createInsertSchema(
  crmQualificationHistoryTable,
).omit({ id: true, createdAt: true });
export type CrmQualificationHistory =
  typeof crmQualificationHistoryTable.$inferSelect;
export type InsertCrmQualificationHistory = z.infer<
  typeof insertCrmQualificationHistorySchema
>;
...
export const insertCrmAlertSchema = createInsertSchema(crmAlertsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertCrmNextStepHistorySchema = createInsertSchema(
  crmNextStepHistoryTable,
).omit({ id: true, createdAt: true });
export type CrmNextStepHistory = typeof crmNextStepHistoryTable.$inferSelect;
export type InsertCrmNextStepHistory = z.infer<
  typeof insertCrmNextStepHistorySchema
>;
...
export const insertCrmSavedViewSchema = createInsertSchema(
  crmSavedViewsTable,
).omit({ id: true, createdAt: true });
...
export type InsertAutomationSequence = z.infer<
  typeof insertAutomationSequenceSchema
>;
...
export const sequenceEnrollmentsTable = pgTable("sequence_enrollments", {
  ...
  sequenceId: integer("sequence_id")
    .notNull()
    .references(() => automationSequencesTable.id, { onDelete: "cascade" }),
  contactId: integer("contact_id")
    .notNull()
    .references(() => crmContactsTable.id, { onDelete: "cascade" }),
  userId: text("user_id").notNull(),
...
export const hubspotSyncStateTable = pgTable(
  "hubspot_sync_state",
  {
    id: serial("id").primaryKey(),
    userId: text("user_id").notNull(),
...
export const hubspotListMappingTable = pgTable(
...
export const crmAuditLogTable = pgTable(
  ...
  (t) => [uniqueIndex("crm_audit_user_idx").on(t.userId, t.createdAt)],
);

export const insertCrmAuditLogSchema = createInsertSchema(
  crmAuditLogTable,
).omit({ id: true, createdAt: true });
...
export const appUserRolesTable = pgTable(
  "app_user_roles",
  {
    id: serial("id").primaryKey(),
    userId: text("user_id").notNull(),
...
export const insertAppUserRoleSchema = createInsertSchema(
  appUserRolesTable,
