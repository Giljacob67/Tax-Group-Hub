...
if (typeof globalThis.DOMMatrix === "undefined") {
  globalThis.DOMMatrix = class DOMMatrix {
    ...
    translate(tx = 0, ty = 0) {
      return this.multiply(new globalThis.DOMMatrix([1, 0, 0, 1, tx, ty]));
    }
    scale(sx = 1, sy = sx) {
      return this.multiply(new globalThis.DOMMatrix([sx, 0, 0, sy, 0, 0]));
    }
    rotate(a) {
      const r = (a * Math.PI) / 180,
        c = Math.cos(r),
        s = Math.sin(r);
      return this.multiply(new globalThis.DOMMatrix([c, s, -s, c, 0, 0]));
    }
    transformPoint(p) {
      return {
    ...
    static fromFloat32Array(a) {
      return new globalThis.DOMMatrix(Array.from(a));
    }
